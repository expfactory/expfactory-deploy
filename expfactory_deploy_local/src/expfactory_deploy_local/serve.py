...
def create_events_file(data, bids_datafile):
    ...
    events_df = events_df.with_columns(
        (pl.col('time_elapsed') - start_trial_end - pl.col('block_duration')).alias('onset')
    )
...
class serve:
    ...
    def POST(self):
        ...
        if web.config.run_num:
            raw_datafile = raw_datafile.replace('.json', f'_run-{web.config.run_num}.json')
            bids_datafile = bids_datafile.replace('.tsv', f'_run-{web.config.run_num}.tsv')

        if web.config.raw_dir:
            if web.config.subject_id:
                sub_dir = os.path.join(web.config.raw_dir, f'sub-{web.config.subject_id}')
                if web.config.session_num:
                    sub_ses_dir = os.path.join(sub_dir, f'ses-{web.config.session_num}', "func")
        ...
        if web.config.bids_dir:
            if web.config.subject_id:
                sub_dir = os.path.join(web.config.bids_dir, f'sub-{web.config.subject_id}')
                if web.config.session_num:
                    sub_ses_dir = os.path.join(sub_dir, f'ses-{web.config.session_num}', "func")
                    os.makedirs(sub_ses_dir, exist_ok=True)
