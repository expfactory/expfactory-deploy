...
class ProlificAPITest(TestCase):
    def setUp(self):
        ...
        self.batt1 = em.Battery.objects.create(
            status="draft", title="test_battery 1", user=self.user
        )
        self.batt2 = em.Battery.objects.create(
            status="draft", title="test_battery 2", user=self.user
        )
        self.study1 = pm.Study.objects.create(
            battery=self.batt1, study_collection=self.scs, rank=0
        )
        self.study2 = pm.Study.objects.create(
            battery=self.batt2, study_collection=self.scs, rank=0
        )
