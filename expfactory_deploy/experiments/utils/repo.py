...
from experiments.models import RepoOrigin, ExperimentRepo
...
def find_new_experiments(search_dir=settings.DEFAULT_EXPERIMENT_DIR):
    ...
    for dir in valid_dirs:
        ...
        repo_origin, repo_created = RepoOrigin.objects.get_or_create(origin=repo.remotes.origin.url, path=repo_path)
        experiment, experiment_created = ExperimentRepo.objects.get_or_create(name=os.path.split(dir)[-1], origin=repo_origin, location=dir)
